

export const FEATURES = [
    { title: 'Answers & Sells', description: 'Replies instantly to "Hm?", "Avail?", and closes deals while you sleep.' },
    { title: 'Reads Photos', description: 'Understand receipts, IDs, and product screenshots instantly.' },
    { title: 'Understands Voice', description: 'Listens to voice messages and replies intelligently.' },
    { title: 'Remembers You', description: 'Remembers past customers so they feel special, not just a number.' },
    { title: 'Always Awake', description: '24/7 online. No lunch breaks, no sleep, no "absent".' },
    { title: 'Fast Setup', description: 'Ready to work for your business in just 3 days.' },
];

export const TEAM = [
    { name: 'Marvin', role: 'CEO, CTO, CFO & Creative Director', image: 'https://i.imgur.com/16N2bPX.png', link: 'https://marvin.orin.work' },
    { name: 'John', role: 'Business Officer', image: 'https://i.imgur.com/ce1YMPv.png', link: '#' },
    { name: 'Gene', role: 'Beta Tester', image: 'https://i.imgur.com/hStVlXl.png', link: '#' },
    { name: 'Roe', role: 'Investor', image: 'https://preview.redd.it/taro-sakamoto-icon-cleaned-edit-and-remastered-by-me-two-v0-kgpamdq4m0rb1.png?width=640&crop=smart&auto=webp&s=405967562d8822280c7ed17334574e3e238720dc', link: '#' }
];

// UPDATED WITH SPECIFIC IMGUR LINKS
export const GALLERY_IMAGES = [
    { 
        urls: [
            "https://i.imgur.com/PhitkXI.png", // Online Sellers
            "https://images.unsplash.com/photo-1566576912321-d58ddd7a6088?q=80&w=1200&auto=format&fit=crop"
        ], 
        caption: "Online Sellers",
        description: "Miss 3AM sales? Orin never sleeps. Catches every inquiry while you're resting. 🚀"
    },
    { 
        urls: [
            "https://i.imgur.com/a7rjEhU.png", // Corporate (UPDATED)
            "https://images.unsplash.com/photo-1497215728101-856f4ea42174?q=80&w=1200&auto=format&fit=crop"
        ], 
        caption: "Corporate",
        description: "Hiring costs ₱226,857/year. Orin costs ₱15,000 monthly. Math doesn't lie. 💡"
    },
    { 
        urls: [
            "https://i.imgur.com/eArcUlU.png", // Airbnb
            "https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?q=80&w=1200&auto=format&fit=crop"
        ], 
        caption: "Airbnb Host",
        description: "Slow replies = lost bookings. Orin responds instantly. Always on point."
    },
    { 
        urls: [
            "https://i.imgur.com/ZevJ53B.png", // LGU
            "https://images.unsplash.com/photo-1573167243872-43c6433b9d40?q=80&w=1200&auto=format&fit=crop"
        ], 
        caption: "LGU & Government",
        description: "Constituents waiting in line? Orin triages 24/7. Zero delays. Efficient Public Service."
    },
    { 
        urls: [
            "https://i.imgur.com/fZZKmj3.png", // Hospitals (UPDATED)
            "https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?q=80&w=1200&auto=format&fit=crop"
        ], 
        caption: "Hospitals",
        description: "Critical info delays cost lives. Orin prioritizes instantly. Every time."
    },
    { 
        urls: [
            "https://i.imgur.com/8KVCQfB.png", // Police
            "https://images.unsplash.com/photo-1596524430615-b46475ddff6e?q=80&w=1200&auto=format&fit=crop"
        ], 
        caption: "Police Stations",
        description: "Emergency response needs speed. Orin handles intake and routing instantly."
    }
];