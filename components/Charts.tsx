...
const dataROI = [ 
    { name: 'Small Biz', val: 2158, color: 'var(--chart-1)' }, 
    { name: 'Medium', val: 10792, color: 'var(--chart-2)' }, 
    { name: 'Large', val: 43171, color: 'var(--chart-3)' }, 
    { name: 'Enterprise', val: 215857, color: 'var(--chart-4)' } 
];
    ...
    if (active && payload && payload.length) {
        return (
            <div className="bg-black/90 border border-white/20 p-4 rounded-xl shadow-xl backdrop-blur-md">
                ...
                <p className="text-[#38F8A8] dark:text-[#38F8A8] text-[#D4AF37] font-mono text-sm">
...
export const MarketGrowthChart = () => {
    // Determine if light mode (naive check, usually passed via context but for simplicity here we check root class or assume CSS vars)
    const isLight = document.documentElement.classList.contains('light') || !document.documentElement.classList.contains('dark');
    const accentColor = isLight ? '#D4AF37' : '#38F8A8'; // Gold vs Green
        ...
        <div className="flex flex-col h-full">
            <div className="h-64 w-full flex-grow">
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={dataGrowth} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke={isLight ? '#E5E7EB' : '#333'} vertical={false} />
                        ...
                        <RechartsTooltip 
                            contentStyle={{ backgroundColor: isLight ? '#FFF' : '#000', border: '1px solid #333', borderRadius: '8px', color: isLight ? '#000' : '#fff' }}
                            itemStyle={{ color: accentColor, fontFamily: 'monospace' }}
                            formatter={(value: number) => [`$${value} Billion`, 'Market Size']}
                        />
                        <Line type="monotone" dataKey="val" stroke={accentColor} strokeWidth={3} dot={{r: 4, fill: accentColor, strokeWidth: 0}} activeDot={{ r: 6, strokeWidth: 0, fill: isLight ? '#000' : '#fff' }} />
                    </LineChart>
                </ResponsiveContainer>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-dashed border-gray-700/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-2">
                ...
                <a 
                    ...
                    target="_blank" 
                    rel="noreferrer"
                    className="group flex items-center gap-1.5 text-[10px] text-gray-500 hover:text-black dark:hover:text-[#38F8A8] transition-colors font-mono uppercase tracking-wider cursor-pointer border-b border-transparent hover:border-black dark:hover:border-[#38F8A8]"
                >
...
export const ROIChart = () => {
    // In a real app, pass theme as prop or context. This is a quick fix to align colors.
    const isLight = !document.documentElement.classList.contains('dark');
    
    // Light Mode: Gold/Black/Gray scale
    // Dark Mode: Neon/Purple/Pink scale
    const colors = isLight 
        ? ['#D4AF37', '#A3862C', '#736024', '#000000'] 
        : ['#38F8A8', '#A855F7', '#FF3366', '#3b82f6'];
            ...
            <div className="h-64 w-full flex-grow">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={dataROI} layout="vertical" margin={{ top: 0, right: 20, left: 10, bottom: 20 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke={isLight ? '#E5E7EB' : '#333'} horizontal={false} />
                        <XAxis 
                            type="number" 
                            stroke="#666" 
                        ...
                        <YAxis 
                            ...
                            tickLine={false}
                        />
                        <RechartsTooltip content={<CustomTooltip />} cursor={{fill: isLight ? 'rgba(0,0,0,0.05)' : 'rgba(255,255,255,0.05)'}} />
                        <Bar dataKey="val" radius={[0, 4, 4, 0]} animationDuration={1500} barSize={20}>
                            {dataROI.map((entry, index) => <Cell key={index} fill={colors[index]} />)}
                        </Bar>
                    </BarChart>
                </ResponsiveContainer>
            </div>
